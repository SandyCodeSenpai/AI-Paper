...
The most influential finding from [sources/llama2-2023](../sources/llama2-2023.md) §3.1 (echoing Zhou et al. 2023, "LIMA"): **the size of the SFT dataset matters less than its quality**. Llama 2 abandoned millions of third-party instruction examples and ended up with **27,540 vendor-written prompt+response pairs**, where the same person wrote both halves. Result: better SFT model than from millions of crowdsourced ones.
...
In multi-turn dialogues, mask all turns except the assistant turn(s) you're training on. [Ghost Attention](ghost-attention.md) takes this further: zero out loss on intermediate assistant turns to avoid learning the synthetic per-turn system prompt.
...
[RLHF](rlhf.md) is what closes those gaps.

## Where it's used

- [sources/llama2-2023](../sources/llama2-2023.md) — the canonical "quality > quantity" SFT recipe
...
- [concepts/rlhf](rlhf.md) — what comes after SFT
- [concepts/ghost-attention](ghost-attention.md) — multi-turn SFT trick
